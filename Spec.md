...
- Text longer than 4,096 Unicode code points is rejected before a worker/model/tool call; accepted text is forwarded exactly without trimming.
...
## 24. Behavioral LLM testing and model selection

`LLMTestingSpec.md` is the authoritative additive specification for offline LLM-boundary tests, the versioned 12-case behavioral dataset, and the opt-in local two-or-more-model benchmark. The default Vitest suite remains deterministic and offline. The separate `npm run eval:models` command accepts a two-model CLI pair through `--model_1` and `--model_2`, or the original `EVAL_MODELS` environment variable for two or more candidates. It uses only configured local Ollama chat models, synthetic fake tools, and isolated in-memory histories; it never changes production model configuration or reads production chat/RAG data. Generated JSON and Markdown reports are ignored under `artifacts/model-evaluation/`.
