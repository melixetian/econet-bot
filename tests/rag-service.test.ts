...
describe("RagService", () => {
  ...
  it("applies distance and complete-chunk context bounds", async () => { const index = new MemoryIndex(); index.rows = [{ text: "12345", filename: "a.txt", chunkIndex: 0, distance: 0.1 }, { text: "67890", filename: "a.txt", chunkIndex: 1, distance: 0.2 }, { text: "far", filename: "a.txt", chunkIndex: 2, distance: 1.1 }]; const service = new RagService(index, new FakeEmbeddings(), { ...testConfig, ragMaxContextChars: 5 }); await expect(service.search("u", "query")).resolves.toEqual({ status: "ok", results: [index.rows[0]], omittedResults: 1 }); });
