...
describe("configuration", () => {
  ...
  it("uses RAG defaults and validates limits", () => { const config = loadWorkerConfig({}); expect(config.agentMaxSteps).toBe(5); expect(config.ragEmbeddingDimension).toBe(768); expect(config.ragChunkOverlapChars).toBe(300); expect(config.ragMaxDistance).toBe(1); expect(() => loadWorkerConfig({ AGENT_MAX_STEPS: "11" })).toThrow("AGENT_MAX_STEPS must be an integer from 1 to 10"); expect(() => loadWorkerConfig({ RAG_CHUNK_SIZE_CHARS: "100", RAG_CHUNK_OVERLAP_CHARS: "100" })).toThrow("RAG_CHUNK_OVERLAP_CHARS"); });
});
